...
class Agent(BaseAgent):
    def __init__(self, agent_type="player", device=None, logger=None, monitor=None):
        torch.manual_seed(0)
        # Force CPU execution to avoid CUDA kernel incompatibility
        # with some platform GPU + PyTorch image combinations.
        ...
        self._feature_offsets = np.cumsum([0] + Config.FEATURES)
    ...
    def predict(self, list_obs_data):
        ...
        logits = self._apply_action_priority_bias(logits, feature_arr, legal_arr)
    ...
    def _apply_action_priority_bias(self, logits, feature, legal_action):
        """Inject hard-priority bias into logits: charge > evade > coverage."""
        if feature.shape[0] < Config.DIM_OF_OBSERVATION:
            return logits

        offs = self._feature_offsets
        global_start = offs[1]
        behavior_start = offs[-3]
        action_priority_start = offs[-2]
        action_priority_end = offs[-1]

        action_priority = feature[action_priority_start:action_priority_end]
        if action_priority.shape[0] != Config.ACTION_NUM:
            return logits

        battery_ratio = float(feature[global_start])
        min_npc_dist_norm = float(feature[behavior_start + 3])
        low_battery_flag = float(feature[behavior_start + 4])

        charge_urgency = max(low_battery_flag, float(np.clip((0.50 - battery_ratio) / 0.25, 0.0, 1.0)))
        evade_urgency = float(np.clip((0.08 - min_npc_dist_norm) / 0.05, 0.0, 1.0))

        logits_out = np.array(logits, dtype=np.float32, copy=True)
        priority_centered = (action_priority - 0.5) * 2.0
        bias_strength = 0.25 + 2.2 * charge_urgency + 1.6 * evade_urgency
        logits_out += bias_strength * priority_centered * legal_action

        if charge_urgency > 0.95 or evade_urgency > 0.95:
            masked_priority = action_priority * legal_action
            best_action = int(np.argmax(masked_priority))
            if legal_action[best_action] > 0:
                logits_out[best_action] += 2.5 + bias_strength
