        ...
        while True:
            ...
            while not done:
                ...
                if done:
                    ...
                    remaining_charge = float(env_info.get("remaining_charge", 0))
                    battery_max = max(float(env_info.get("battery_max", 200)), 1.0)
                    remaining_ratio = remaining_charge / battery_max

                    if truncated:
                        # Survived to max steps: higher cleaning ratio → more reward
                        # 存活到最大步数：清扫比例越高奖励越多
                        cleaning_ratio = fm.dirt_cleaned / max(fm.total_dirt, 1)
                        final_reward = 5.0 + 5.0 * cleaning_ratio - 1.5 * remaining_ratio
                        result_str = "WIN"
                    else:
                        ...
                        if remaining_charge > 0:
                            final_reward -= 2.0 + 2.0 * remaining_ratio
                        result_str = "FAIL"
