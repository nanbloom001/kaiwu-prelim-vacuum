        ...
        while True:
            ...
            while not done:
                ...
                if done:
                    ...
                    episode_metrics = fm.get_episode_metrics()

                    if truncated:
                        cleaning_ratio = fm.dirt_cleaned / max(fm.total_dirt, 1)
                        charge_bonus = min(2.0, 0.5 * episode_metrics["charge_success_cnt"])
                        final_reward = 8.0 + 12.0 * cleaning_ratio + charge_bonus
                        result_str = "WIN"
                    else:
                        fail_reason = str(getattr(fm, "_fail_reason", ""))
                        if "battery" in fail_reason:
                            final_reward = -12.0
                        elif "collision" in fail_reason:
                            final_reward = -14.0
                        else:
                            final_reward = -8.0
                        result_str = "FAIL"
                    ...
                    self.logger.info(
                        ...
                        f"coverage:{episode_metrics['coverage_rate']:.4f} "
                        f"repeat:{episode_metrics['repeat_visit_ratio']:.4f} "
                        f"charge_succ:{int(episode_metrics['charge_success_cnt'])} "
                        f"stuck_cnt:{int(episode_metrics['stuck_cnt'])}"
                    )
                        ...
                        self.monitor.put_data(
                            {
                                os.getpid(): {
                                    ...
                                    "clean_ratio": episode_metrics["clean_ratio"],
                                    "repeat_visit_ratio": episode_metrics["repeat_visit_ratio"],
                                    "charge_success_cnt": episode_metrics["charge_success_cnt"],
                                    "charge_success_rate": episode_metrics["charge_success_rate"],
                                    "low_battery_trigger_cnt": episode_metrics["low_battery_trigger_cnt"],
                                    "near_npc_steps": episode_metrics["near_npc_steps"],
                                    "stuck_cnt": episode_metrics["stuck_cnt"],
                                }
                            }
                        )
