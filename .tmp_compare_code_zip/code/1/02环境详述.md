# 环境详述
在开始开发前，请仔细阅读腾讯开悟强化学习开发框架，深入理解**环境、智能体、工作流**等核心概念及其相关接口的使用方法。
...
### 💡 补充说明
1. `train_env_conf.toml` 文件中的配置仅在训练时生效，请按上表数据描述进行配置。若配置错误，训练任务会变为**失败**状态，此时可以通过查看 `env` 模块的错误日志进行排查。
2. 若需调整模型评估任务的配置，用户需要通过腾讯开悟平台创建评估任务并完成环境配置，详细参数见智能体详述模型评估。

### 默认配置（train_env_conf.toml）
```toml
[env_conf]
...
### 移动规则
1. **直线移动**：目标格子可通行即可移动
2. **斜向移动防穿角**：目标格子可通行，且相邻两条边（水平/垂直方向）至少有一条可通行，避免卡角穿墙
3. **碰撞处理**：若目标格子不可通行或斜向移动条件不满足，智能体停留原地，但**仍消耗 1 步和 1 电量**
...
### 总结
1. 环境通过`train_env_conf.toml`配置核心参数，训练与评估配置方式分离
2. 智能体观测范围固定为21×21，动作空间为8方向移动，移动失败仍消耗步数与电量
3. 任务得分仅统计小悟机器人清扫的污渍格数，与强化学习reward概念区分