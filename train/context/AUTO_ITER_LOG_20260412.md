...
```
[快照 08:03] ep:1079 | avg_score:210 | FAIL:93% | avg_invalid:0.305 | charge:0/interval
```

**趋势**: Prev30=156 → Last30=210 (+34.6%! ⬆️⬆️)
**Top 5**: 415(0.126,map6), 414(0.146,map5), 377(0.000,map6 WIN!), 364(0.004,map7), 352(0.107,map1)
**WIN**: **2/30** (ep:1058 map:6 score=377, ep:1073 map:1 score=289) | **灾难局**(inv>0.9): **0/30** (首次清零!) | **Good局**(inv<0.2): **10/30** (33%)

**训练算法metrics** (global_step: ~13900):
- value_loss: 120-150 (稳定✓)
- entropy_loss: 0.228~0.327 (10次均值=0.275, 有所回升✓)
- policy_loss: -2~-5

**关键变化**:
1. **灾难局清零**: inv>0.9从17%降至0%, 这是session内首次!
2. **分数大幅跃升**: 210远超Q3 peak(179), 接近session最高
3. **2次WIN**: map6(map6曾是弱图!)和map1获胜
4. **新强分**: 415, 414 — 接近旧最佳450

**可能解释**: entropy虽低但模型找到了稳定的清扫策略, 恰好近期map/profile组合有利. 灾难局清零可能受益于NPC位置随机性.

**决策**: 不做调整. 训练处于最佳状态. BETA=0.007仍建议重启时应用(长期防收敛), 但当前无需紧急干预.
