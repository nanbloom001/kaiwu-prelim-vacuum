...
class Preprocessor:
    ...
    def _build_charger_candidates(self, dist):
        ...
        for score, charger, goal, goal_dist in rankings[: Config.MAX_CHARGER_CANDIDATES]:
            out.append(
                {
                    "type": "charger",
                    "goal": tuple(goal),
    ...
    def _rank_charger_candidates(self, dist):
        ...
        for charger in self.chargers:
            ...
            if int(charger.get("id", -1)) == int(self.last_charger_id):
                score -= Config.CHARGE_REPEAT_MULTIPLIER
            ranked.append((float(score), charger, tuple(best_goal), int(best_dist)))
        ranked.sort(key=lambda item: item[0], reverse=True)
        return ranked
    ...
    def _charger_retry_margin_for_goal(self, goal):
        ...
        if not path:
            return float(Config.RETURN_CHARGE_BUFFER)
        turns = 0
        narrow = 0
        blocked = 0
        last_delta = None
    ...
    def _post_charge_goal_penalty(self, pos):
        ...
        return Config.POST_CHARGE_HALO_PENALTY * scale
    ...
    def _bind_active_region(self, region, preserve_goal=None):
        ...
        self.active_cover_sequence = self._build_cover_sequence(region)
        self.active_cover_index = 0
        self.active_cover_strip = 0
    ...
    def _dynamic_return_buffer(self, pos, charger_dist=None):
        x, z = int(pos[0]), int(pos[1])
        buffer = float(Config.RETURN_CHARGE_BUFFER)
    ...
    def _pick_region_goal(self, region, dist, charger_dist):
        ...
        for pos in region["cells"]:
            ...
            if self.active_cover_sequence and self.active_cover_index < len(self.active_cover_sequence):
                ...
                elif self.active_cover_index + 1 < len(self.active_cover_sequence) and pos == tuple(self.active_cover_sequence[self.active_cover_index + 1]):
    ...
    def _style_factor(self, key):
        ...
        return float(table.get(style, table["balanced"]).get(key, 1.0))
                    ...
                    if in_region_next:
                        ...
                        if self.active_cover_sequence:
                            ...
                            if tuple(next_pos) == tuple(current_cover):
                                score += Config.ROOM_SWEEP_PERSIST_BONUS
                            elif self.active_cover_index + 1 < len(self.active_cover_sequence) and tuple(next_pos) == tuple(self.active_cover_sequence[self.active_cover_index + 1]):
                                score += Config.ROOM_SWEEP_NEXT_STRIP_BONUS
