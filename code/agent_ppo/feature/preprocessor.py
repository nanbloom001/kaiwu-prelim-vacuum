...
class Preprocessor:
    ...
    def _nearest_charger_distance(self) -> float:
        ...
        return float(
            min(np.sqrt((cx - hx) ** 2 + (cz - hz) ** 2) for cx, cz in self.charger_positions)
        )
    ...
    def _nearest_unarrived_charger_distance(self) -> float:
        ...
        return float(
            min(np.sqrt((cx - hx) ** 2 + (cz - hz) ** 2) for cx, cz in unarrived_centers)
        )
    ...
    def _resolve_pending_arrivals(
        ...
        *,
        ...
        for region_key, info in list(self.pending_arrivals.items()):
            ...
            severe_risk = is_starving or charge_loop_failure or battery_ratio <= 0.18
            ...
            if info["final_confirmed"]:
                if charge_loop_failure and age <= int(params["final_deadline"]) + 40:
    ...
    def finalize_episode_rewards(
        ...
        *,
        ...
        reward = 0.0
        is_charge_fail = result_str == "FAIL" and (final_mode == "charge" or bool(charge_fail_after_arrival))
            ...
            if not info["final_confirmed"]:
                ...
                if is_charge_fail and paid > 0.0:
                    ...
                    self.arrival_confirm_to_fail_gap = (
                        gap if self.arrival_confirm_to_fail_gap < 0 else min(self.arrival_confirm_to_fail_gap, gap)
                    )
    ...
    def reward_process(self) -> float:
        ...
        is_starving = charger_known and not is_on_charger and float(self.battery) <= nearest_charger_dist + 22.0
        ...
        unarrived_progress = self._last_nearest_unarrived_charger_dist - self._nearest_unarrived_charger_dist
        if (
            charger_known
            and not is_starving
