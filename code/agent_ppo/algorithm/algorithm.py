...
class Algorithm:
    ...
    def _unpack_batch_tensor(self, batch_data):
        batch_tensor = torch.from_numpy(batch_data) if isinstance(batch_data, np.ndarray) else batch_data
        ...
        begin += Config.ACTION_NUM
        planner_prob = batch_tensor[:, begin : begin + Config.ACTION_NUM]
        begin += Config.ACTION_NUM
        mix_alpha = batch_tensor[:, begin : begin + Config.VALUE_NUM]
    ...
    def _unpack_sample_objects(self, list_sample_data):
        ...
        planner_prob = torch.stack([s.planner_prob for s in list_sample_data]).to(**to_device)
        mix_alpha = torch.stack([s.mix_alpha for s in list_sample_data]).to(**to_device)
