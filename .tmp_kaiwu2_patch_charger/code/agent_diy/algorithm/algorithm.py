...
class Algorithm:
    ...
    def learn(self, list_sample_data):
        if not list_sample_data:
            return {
                ...
                "entropy_loss": 0.0,
                "imitation_loss": 0.0,
                "teacher_mix": float(self.get_teacher_mix()),
                "imitation_coef": float(self.get_imitation_coef()),
                "teacher_weight": 0.0,
                "policy_weight": 0.0,
            }
        ...
        if now - self.last_report_time >= 60:
            if self.logger:
                self.logger.info(
                    "policy_loss: %.4f, decision_policy_loss: %.4f, style_policy_loss: %.4f, value_loss: %.4f, entropy_loss: %.4f, imitation_loss: %.4f, teacher_mix: %.4f, imitation_coef: %.4f"
                    % (
                        results["policy_loss"],
            ...
            if self.monitor:
                self.monitor.put_data({os.getpid(): _scalarize_monitor_metrics(results)})
    ...
    def _compute_loss(
        ...
        return total_loss, {
            "policy_loss": float((decision_policy_loss + Config.STYLE_POLICY_COEF * style_policy_loss).item()),
